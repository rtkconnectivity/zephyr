...
__subsystem struct ir_driver_api {
	...
	// void (*set_tx_cb)(const struct device *dev, ir_callback_t callback, void *user_data);
	/** IR rx function */
	int (*rx_enable)(const struct device *dev, ir_callback_t callback, void *user_data,
			 uint32_t rx_len, uint32_t *buf, uint32_t idle_cnt);
	/** Stop ir rx and receive remaining data */
	int (*rx_disable)(const struct device *dev, struct ir_event_rx *data);
	// void (*set_rx_cb)(const struct device *dev, ir_callback_t callback, void *user_data);

#ifdef CONFIG_IR_ASYNC_API
	/** IR tx function */
	/** IR rx function */
#endif
};
...
static inline int z_impl_ir_rx_enable(const struct device *dev, ir_callback_t callback, void *user_data,
			 uint32_t rx_len, uint32_t *buf, uint32_t idle_cnt)
{
	const struct ir_driver_api *api = (const struct ir_driver_api *)dev->api;

	if (api->rx_enable != NULL) {
		return api->rx_enable(dev, callback, user_data, rx_len, buf, idle_cnt);
	}
...
/**
 * @brief Set the interrupt callback function pointer for ir tx.
 *
 * This sets up the callback for ir tx interrupt. When the ir tx completes,
 * the specified function will be called with specified user data.
 *
 ...
 * @retval -ENOSYS If this function is not implemented.
 */
// static inline int ir_set_tx_cb(const struct device *dev, ir_tx_callback_t callback, void
// *user_data)
// {
// 	const struct ir_driver_api *api =
// 	(const struct ir_driver_api *)dev->api;

// 	if ((api != NULL) && (api->set_tx_cb != NULL)) {
// 		api->set_tx_cb(dev, callback, user_data);
// 		return 0;
// 	} else {
// 		return -ENOSYS;
// 	}
...
/**
 * @brief Set the interrupt callback function pointer for ir rx.
 *
 * This sets up the callback for ir rx interrupt. When the ir rx receives
 * enough data or when the ir rx stops, the specified function will be
 * called with specified user data.
 ...
 * @retval -ENOSYS If this function is not implemented.
 */
// static inline int ir_set_rx_cb(const struct device *dev, ir_rx_callback_t callback, void
// *user_data)
// {
// 	const struct ir_driver_api *api =
// 	(const struct ir_driver_api *)dev->api;

// 	if ((api != NULL) && (api->set_rx_cb != NULL)) {
// 		api->set_rx_cb(dev, callback, user_data);
// 		return 0;
// 	} else {
// 		return -ENOSYS;
// 	}
