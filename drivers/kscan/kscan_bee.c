...
LOG_MODULE_REGISTER(kscan_bee, CONFIG_KSCAN_LOG_LEVEL);
...
static int kscan_bee_configure(const struct device *dev, kscan_callback_t callback)
{
...
static int kscan_bee_disable_callback(const struct device *dev)
{
	struct kscan_bee_data *data = dev->data;
...
static int kscan_bee_enable_callback(const struct device *dev)
{
	struct kscan_bee_data *data = dev->data;
...
static const struct kscan_driver_api kscan_bee_driver_api = {
	.config = kscan_bee_configure,
	.disable_callback = kscan_bee_disable_callback,
	.enable_callback = kscan_bee_enable_callback,
};
...
#define BEE_KSCAN_INIT(index)                                                                      \
	...
	DEVICE_DT_INST_DEFINE(index, &kscan_bee_init, PM_DEVICE_DT_INST_GET(index),                \
			      &kscan_bee_data_##index, &kscan_bee_cfg_##index, APPLICATION,        \
			      CONFIG_KSCAN_INIT_PRIORITY, &kscan_bee_driver_api);                  \
                                                                                                   \
