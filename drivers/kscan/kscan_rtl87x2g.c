...
static void keyscan_timer_cb(struct k_timer *timer);

static K_TIMER_DEFINE(keyscan_timer, keyscan_timer_cb, NULL);
...
static void kscan_rtl87x2g_isr(const struct device *dev)
{
    ...
    {
        LOG_ERR("kscan_rtl87x2g_isr:scan end; press key num is %d", new_press_num);
        if(new_press_num == 0) {
#ifdef CONFIG_PM_DEVICE
            ...
            LOG_ERR("stop manual scan timer");
            k_timer_stop(&keyscan_timer);
        } else {
            LOG_ERR("start manual scan timer");
            k_timer_start(&keyscan_timer, K_MSEC(CONFIG_RTL87X2G_KEYSCAN_MANUAL_SCAN_INTERVAL_MSEC),K_MSEC(CONFIG_RTL87X2G_KEYSCAN_MANUAL_SCAN_INTERVAL_MSEC));
        }
...
{
    LOG_ERR("kscan pm check flag is %d",kscan_pm_check_state);
...
static void keyscan_timer_cb(struct k_timer *timer) {
    LOG_ERR("manual_keyscan_timer_cb");
#ifdef CONFIG_PM_DEVICE
                ...
                if(wakeup_flag) {
                    if (data->press_rows & BIT(j)) {
                        if (state->pins[i].wakeup_high) {
                            ...
                            LOG_ERR("pm suspend:  there is pressed key row%d, pin%d,long pressed %d",
                                i, state->pins[i].pin);
                            Pad_SetControlMode(state->pins[i].pin, PAD_SW_MODE);
            ...
            for (uint8_t i = 0U; i < state->pin_cnt; i++)
            {
                if (state->pins[i].wakeup_low || state->pins[i].wakeup_high)
                {
                    if (System_WakeUpInterruptValue(state->pins[i].pin) == SET)
                    {
                        ...
                        /* register trigger manual mode init */
                        //k_timer_stop(&keyscan_timer);
                        LOG_ERR("system wkup pin %d, trig manual scan once",state->pins[i].pin);
