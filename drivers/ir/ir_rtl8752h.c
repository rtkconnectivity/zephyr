...
#if IR_HAS_TX_DMA
void ir_rtl8752h_dma_tx_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
			   int status)
{
#if DBG_DIRECT_SHOW
...
#if IR_HAS_RX_DMA
void ir_rtl8752h_dma_rx_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
			   int status)
{
#if DBG_DIRECT_SHOW
	DBG_DIRECT("[%s] line%d", __func__, __LINE__);
	...
	const struct ir_rtl8752h_config *cfg = dev->config;
...
static int ir_rtl8752h_tx_init(const struct device *dev)
{
	struct ir_rtl8752h_data *data = dev->data;
	int err;
...
static int ir_rtl8752h_rx_init(const struct device *dev)
{
	struct ir_rtl8752h_data *data = dev->data;
	int err;
...
static int ir_rtl8752h_rx_enable(const struct device *dev, ir_callback_t callback, void *user_data,
...
#ifdef CONFIG_PM_DEVICE
static int ir_rtl8752h_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct ir_rtl8752h_config *config = dev->config;
	struct ir_rtl8752h_data *data = dev->data;
...
#define IR_DMA_CHANNEL_INIT(index, dir)                                                            \
	...
	.dma_cfg =                                                                                 \
		{                                                                                  \
			...
			.dma_callback = ir_rtl8752h_dma_##dir##_cb,                                \
	},                                                                                         \
	.src_addr_increment =                                                                      \
