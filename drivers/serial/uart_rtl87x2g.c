...
static int uart_rtl87x2g_configure(const struct device *dev, const struct uart_config *cfg)
{
...
static int uart_rtl87x2g_config_get(const struct device *dev, struct uart_config *cfg)
{
	struct uart_rtl87x2g_data *data = dev->data;
...
static void uart_rtl87x2g_poll_out(const struct device *dev, unsigned char c)
{
	const struct uart_rtl87x2g_config *config = dev->config;
	UART_TypeDef *uart = config->uart;
...
static int uart_rtl87x2g_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	const struct uart_rtl87x2g_config *config = dev->config;
	UART_TypeDef *uart = config->uart;
	uint8_t num_tx = 0U;
...
static int uart_rtl87x2g_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	const struct uart_rtl87x2g_config *config = dev->config;
	UART_TypeDef *uart = config->uart;
	uint8_t num_rx = 0U;
...
#endif
#ifdef CONFIG_UART_ASYNC_API
static inline void async_user_callback(struct uart_rtl87x2g_data *data, struct uart_event *event)
{
	if (data->async_cb) {
...
void uart_rtl87x2g_dma_tx_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
			     int status)
{
...
void uart_rtl87x2g_dma_rx_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
			     int status)
{
#if DBG_DIRECT_SHOW
...
static int uart_rtl87x2g_async_callback_set(const struct device *dev, uart_callback_t callback,
					    void *user_data)
{
#if DBG_DIRECT_SHOW
	DBG_DIRECT("[%s]", __func__);
...
static int uart_rtl87x2g_async_tx(const struct device *dev, const uint8_t *tx_data, size_t buf_size,
				  int32_t timeout)
{
#if DBG_DIRECT_SHOW
...
static int uart_rtl87x2g_async_rx_enable(const struct device *dev, uint8_t *rx_buf, size_t buf_size,
					 int32_t timeout)
{
#if DBG_DIRECT_SHOW
...
static int uart_rtl87x2g_async_rx_buf_rsp(const struct device *dev, uint8_t *buf, size_t len)
{
#if DBG_DIRECT_SHOW
...
static void uart_rtl87x2g_async_tx_timeout(struct k_work *work)
{
	...
	struct uart_rtl87x2g_data *data =
		CONTAINER_OF(tx_stream, struct uart_rtl87x2g_data, dma_tx);
...
static void uart_rtl87x2g_async_rx_timeout(struct k_work *work)
{
	...
	struct uart_dma_stream *rx_stream =
		CONTAINER_OF(dwork, struct uart_dma_stream, timeout_work);
	struct uart_rtl87x2g_data *data =
		CONTAINER_OF(rx_stream, struct uart_rtl87x2g_data, dma_rx);
	const struct device *dev = data->dev;
...
static int uart_rtl87x2g_async_init(const struct device *dev)
{
	...
	k_work_init_delayable(&data->dma_rx.timeout_work, uart_rtl87x2g_async_rx_timeout);
	k_work_init_delayable(&data->dma_tx.timeout_work, uart_rtl87x2g_async_tx_timeout);
...
#define RTL87X2G_UART_IRQ_HANDLER(index)                                                           \
	static void uart_rtl87x2g_irq_config_func_##index(const struct device *dev)                \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), uart_rtl87x2g_isr,  \
...
#define RTL87X2G_UART_INIT(index)                                                                  \
	...
	DEVICE_DT_INST_DEFINE(index, &uart_rtl87x2g_init, PM_DEVICE_DT_INST_GET(index),            \
