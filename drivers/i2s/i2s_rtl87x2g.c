...
/**
 * @brief Driver for I2S port on RTL87X2G family processor.
...
#if defined(CONFIG_SOC_SERIES_RTL87X2G)
#define RTL87X2G_I2S_TX_CH_L_R    I2S_CH_L_R
#define RTL87X2G_I2S_TX_CH_R_L    I2S_CH_R_L
#define RTL87X2G_I2S_TX_CH_L_L    I2S_CH_L_L
#define RTL87X2G_I2S_TX_CH_R_R    I2S_CH_R_R
#define RTL87X2G_I2S_RX_CH_L_R    I2S_CH_L_R
#define RTL87X2G_I2S_RX_CH_R_L    I2S_CH_R_L
#define RTL87X2G_I2S_RX_CH_L_L    I2S_CH_L_L
#define RTL87X2G_I2S_RX_CH_R_R    I2S_CH_R_R
#define RTL87X2G_I2S_TX_MSB_First I2S_MSB_First
#define RTL87X2G_I2S_TX_LSB_First I2S_LSB_First
#define RTL87X2G_I2S_RX_MSB_First I2S_MSB_First
#define RTL87X2G_I2S_RX_LSB_First I2S_LSB_First
#define RTL87X2G_I2S_TX_FIFO_ADDR I2S_TX_FIFO_WR_ADDR
#define RTL87X2G_I2S_RX_FIFO_ADDR I2S_RX_FIFO_RD_ADDR

#define RTL87X2G_I2S_WithExtCodecCmd(i2s, cmd) I2S_WithExtCodecCmd(i2s, cmd)
#endif

LOG_MODULE_REGISTER(i2s_rtl87x2g, CONFIG_I2S_LOG_LEVEL);

#define DT_DRV_COMPAT realtek_rtl87x2g_i2s

#define I2S_SRC_CLK 40000000

#if !defined(CONFIG_I2S_RTL87X2G_RX) && !defined(CONFIG_I2S_RTL87X2G_TX)
...
struct i2s_rtl87x2g_data {
#if defined(CONFIG_I2S_RTL87X2G_TX)
	struct stream dma_tx;
	void *tx_in_msgs[CONFIG_I2S_RTL87X2G_TX_BLOCK_COUNT];
	void *tx_out_msgs[CONFIG_I2S_RTL87X2G_TX_BLOCK_COUNT];
#endif
#if defined(CONFIG_I2S_RTL87X2G_RX)
	struct stream dma_rx;
	void *rx_in_msgs[CONFIG_I2S_RTL87X2G_RX_BLOCK_COUNT];
	void *rx_out_msgs[CONFIG_I2S_RTL87X2G_RX_BLOCK_COUNT];
#endif
};

#if defined(CONFIG_I2S_RTL87X2G_TX)
static void i2s_rtl87x2g_dma_tx_cb(const struct device *, void *, uint32_t, int);
static void i2s_tx_stream_disable(const struct device *, bool drop);
#endif
#if defined(CONFIG_I2S_RTL87X2G_RX)
static void i2s_rtl87x2g_dma_rx_cb(const struct device *, void *, uint32_t, int);
...
static inline void i2s_set_satus(struct i2s_rtl87x2g_data *dev_data, enum i2s_dir dir,
...
{
	const struct i2s_rtl87x2g_config *dev_cfg = dev->config;
	struct i2s_rtl87x2g_data *dev_data = dev->data;
...
static void i2s_rtl87x2g_dma_tx_cb(const struct device *dma_dev, void *arg, uint32_t channel,
				   int status)
{
...
static int i2s_rtl87x2g_write(const struct device *dev, void *mem_block, size_t size)
{
	struct i2s_rtl87x2g_data *dev_data = dev->data;
	struct stream *strm = &dev_data->dma_tx;
	int ret;

	LOG_DBG("i2s_rtl87x2g_write");
...
static int i2s_rtl87x2g_read(const struct device *dev, void **mem_block, size_t *size)
{
	struct i2s_rtl87x2g_data *dev_data = dev->data;
	struct stream *strm = &dev_data->dma_rx;
	void *buffer;
...
static void i2s_rtl87x2g_dma_rx_cb(const struct device *dma_dev, void *arg, uint32_t channel,
				   int status)
{
...
static int i2s_rtl87x2g_configure(const struct device *dev, enum i2s_dir dir,
				  const struct i2s_config *i2s_cfg)
{
	...
	if ((i2s_cfg->format & I2S_FMT_DATA_ORDER_LSB) == I2S_FMT_DATA_ORDER_LSB) {
		I2S_InitStruct.I2S_TxBitSequence = RTL87X2G_I2S_TX_LSB_First;
		I2S_InitStruct.I2S_RxBitSequence = RTL87X2G_I2S_RX_LSB_First;
	} else {
		I2S_InitStruct.I2S_TxBitSequence = RTL87X2G_I2S_TX_MSB_First;
		I2S_InitStruct.I2S_RxBitSequence = RTL87X2G_I2S_RX_MSB_First;
	}
	...
	I2S_InitStruct.I2S_TxChSequence = RTL87X2G_I2S_TX_CH_L_R;
	I2S_InitStruct.I2S_RxChSequence = RTL87X2G_I2S_RX_CH_L_R;
#if defined(CONFIG_I2S_RTL87X2G_TX)
#if defined(CONFIG_SOC_SERIES_RTL87X2G)
	...
	/* clock */
	(void)clock_control_on(RTL87X2G_CLOCK_CONTROLLER, (clock_control_subsys_t)&dev_cfg->clkid);

	I2S_Init(base, &I2S_InitStruct);

	RTL87X2G_I2S_WithExtCodecCmd(base, (!dev_cfg->internal_codec));
...
static const struct i2s_config *i2s_rtl87x2g_config_get(const struct device *dev, enum i2s_dir dir)
{
	struct i2s_rtl87x2g_data *dev_data = dev->data;
...
static int i2s_rtl87x2g_trigger(const struct device *dev, enum i2s_dir dir,
...
static void i2s_rtl87x2g_isr(const struct device *dev)
{
	ARG_UNUSED(dev);
}

static int i2s_rtl87x2g_init(const struct device *dev)
{
	const struct i2s_rtl87x2g_config *dev_cfg = dev->config;
	struct i2s_rtl87x2g_data *dev_data = dev->data;
	I2S_TypeDef *base = (I2S_TypeDef *)dev_cfg->base;
	...
	k_msgq_init(&dev_data->dma_tx.in_queue, (char *)dev_data->tx_in_msgs, sizeof(void *),
		    CONFIG_I2S_RTL87X2G_TX_BLOCK_COUNT);
	...
	dev_data->dma_tx.blk_cfg[0].dest_address = (uint32_t)(&(base->RTL87X2G_I2S_TX_FIFO_ADDR));
	...
	dev_data->dma_tx.blk_cfg[1].dest_address = (uint32_t)(&(base->RTL87X2G_I2S_TX_FIFO_ADDR));
	...
	k_msgq_init(&dev_data->dma_rx.in_queue, (char *)dev_data->rx_in_msgs, sizeof(void *),
		    CONFIG_I2S_RTL87X2G_RX_BLOCK_COUNT);
	...
	dev_data->dma_rx.blk_cfg[0].source_address = (uint32_t)(&(base->RTL87X2G_I2S_RX_FIFO_ADDR));
	...
	dev_data->dma_rx.blk_cfg[1].source_address = (uint32_t)(&(base->RTL87X2G_I2S_RX_FIFO_ADDR));
...
static const struct i2s_driver_api i2s_rtl87x2g_driver_api = {
	.configure = i2s_rtl87x2g_configure,
#if defined(CONFIG_I2S_RTL87X2G_RX)
	.read = i2s_rtl87x2g_read,
#endif
#if defined(CONFIG_I2S_RTL87X2G_TX)
	.write = i2s_rtl87x2g_write,
#endif
	.config_get = i2s_rtl87x2g_config_get,
	.trigger = i2s_rtl87x2g_trigger,
};
...
#define I2S_DMA_CHANNEL_INIT(index, dir)                                                           \
	...
	.dma_cfg =                                                                                 \
		{                                                                                  \
			...
			.dma_callback = i2s_rtl87x2g_dma_##dir##_cb,                               \
			.cyclic = true,                                                            \
	},                                                                                         \
	.src_addr_increment =                                                                      \
		RTL87X2G_DMA_CONFIG_SOURCE_ADDR_INC(RTL87X2G_DMA_CHANNEL_CONFIG(index, dir)),      \
	.dst_addr_increment =                                                                      \
		RTL87X2G_DMA_CONFIG_DESTINATION_ADDR_INC(RTL87X2G_DMA_CHANNEL_CONFIG(index, dir)),

#define I2S_DMA_CHANNEL(index, dir)                                                                \
	.dma_##dir = {COND_CODE_1(DT_INST_DMAS_HAS_NAME(index, dir),                               \
				  (I2S_DMA_CHANNEL_INIT(index, dir)), (NULL))},

#if defined(CONFIG_I2S_RTL87X2G_TX) && defined(CONFIG_I2S_RTL87X2G_RX)
#define I2S_DMA_INIT(index) I2S_DMA_CHANNEL(index, tx) I2S_DMA_CHANNEL(index, rx)
#elif defined(CONFIG_I2S_RTL87X2G_TX)
#define I2S_DMA_INIT(index) I2S_DMA_CHANNEL(index, tx)
#elif defined(CONFIG_I2S_RTL87X2G_RX)
#define I2S_DMA_INIT(index) I2S_DMA_CHANNEL(index, rx)
#endif

#define RTL87X2G_I2S_INIT(index)                                                                   \
	static void i2s_rtl87x2g_irq_config_func_##index(const struct device *dev);                \
                                                                                                   \
	PINCTRL_DT_INST_DEFINE(index);                                                             \
                                                                                                   \
	static const struct i2s_rtl87x2g_config i2s_rtl87x2g_cfg_##index = {                       \
		.base = (I2S_TypeDef *)DT_INST_REG_ADDR(index),                                    \
		.clkid = DT_INST_CLOCKS_CELL(index, id),                                           \
		.irq_config_func = i2s_rtl87x2g_irq_config_func_##index,                           \
		.pinctrl = PINCTRL_DT_INST_DEV_CONFIG_GET(index),                                  \
		.internal_codec = DT_NODE_HAS_COMPAT_STATUS(DT_INST_CHILD(index, codec_0),         \
							    realtek_rtl87x2g_codec, okay),         \
	};                                                                                         \
                                                                                                   \
	static struct i2s_rtl87x2g_data i2s_rtl87x2g_data_##index = {I2S_DMA_INIT(index)};         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(index, &i2s_rtl87x2g_init, NULL, &i2s_rtl87x2g_data_##index,         \
			      &i2s_rtl87x2g_cfg_##index, POST_KERNEL, CONFIG_I2S_INIT_PRIORITY,    \
			      &i2s_rtl87x2g_driver_api);                                           \
                                                                                                   \
	static void i2s_rtl87x2g_irq_config_func_##index(const struct device *dev)                 \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(index, 0, irq),                                     \
			    DT_INST_IRQ_BY_IDX(index, 0, priority), i2s_rtl87x2g_isr,              \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		irq_enable(DT_INST_IRQN(index));                                                   \
	}

DT_INST_FOREACH_STATUS_OKAY(RTL87X2G_I2S_INIT)
