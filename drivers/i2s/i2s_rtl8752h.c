...
/**
 * @brief Driver for I2S port on RTL8752H family processor.
...
LOG_MODULE_REGISTER(i2s_rtl8752h, CONFIG_I2S_LOG_LEVEL);

#define DT_DRV_COMPAT realtek_rtl8752h_i2s

#define I2S_SRC_CLK 40000000

#if !defined(CONFIG_I2S_RTL8752H_RX) && !defined(CONFIG_I2S_RTL8752H_TX)
...
struct i2s_rtl8752h_data {
#if defined(CONFIG_I2S_RTL8752H_TX)
	struct stream dma_tx;
	void *tx_in_msgs[CONFIG_I2S_RTL8752H_TX_BLOCK_COUNT];
	void *tx_out_msgs[CONFIG_I2S_RTL8752H_TX_BLOCK_COUNT];
#endif
#if defined(CONFIG_I2S_RTL8752H_RX)
	struct stream dma_rx;
	void *rx_in_msgs[CONFIG_I2S_RTL8752H_RX_BLOCK_COUNT];
	void *rx_out_msgs[CONFIG_I2S_RTL8752H_RX_BLOCK_COUNT];
#endif
};

#if defined(CONFIG_I2S_RTL8752H_TX)
static void i2s_rtl8752h_dma_tx_cb(const struct device *, void *, uint32_t, int);
static void i2s_tx_stream_disable(const struct device *, bool drop);
#endif
#if defined(CONFIG_I2S_RTL8752H_RX)
static void i2s_rtl8752h_dma_rx_cb(const struct device *, void *, uint32_t, int);
...
static inline void i2s_set_satus(struct i2s_rtl8752h_data *dev_data, enum i2s_dir dir,
...
{
	const struct i2s_rtl8752h_config *dev_cfg = dev->config;
	struct i2s_rtl8752h_data *dev_data = dev->data;
...
static void i2s_rtl8752h_dma_tx_cb(const struct device *dma_dev, void *arg, uint32_t channel,
				   int status)
{
...
static int i2s_rtl8752h_write(const struct device *dev, void *mem_block, size_t size)
{
	struct i2s_rtl8752h_data *dev_data = dev->data;
	struct stream *strm = &dev_data->dma_tx;
	int ret;

	LOG_DBG("i2s_rtl8752h_write");
...
static int i2s_rtl8752h_read(const struct device *dev, void **mem_block, size_t *size)
{
	struct i2s_rtl8752h_data *dev_data = dev->data;
	struct stream *strm = &dev_data->dma_rx;
	void *buffer;
...
static void i2s_rtl8752h_dma_rx_cb(const struct device *dma_dev, void *arg, uint32_t channel,
				   int status)
{
...
static int i2s_rtl8752h_configure(const struct device *dev, enum i2s_dir dir,
				  const struct i2s_config *i2s_cfg)
{
	...
	if (i2s_cfg->frame_clk_freq == 8000) {
		...
		(void)clock_control_off(RTL8752H_CLOCK_CONTROLLER,
	...
	/* clock */
	(void)clock_control_on(RTL8752H_CLOCK_CONTROLLER, (clock_control_subsys_t)&dev_cfg->clkid);
...
const struct i2s_config *i2s_rtl8752h_config_get(const struct device *dev, enum i2s_dir dir)
{
	struct i2s_rtl8752h_data *dev_data = dev->data;
...
static int i2s_rtl8752h_trigger(const struct device *dev, enum i2s_dir dir,
...
static void i2s_rtl8752h_isr(const struct device *dev)
{
	const struct i2s_rtl8752h_config *dev_cfg = dev->config;
	struct i2s_rtl8752h_data *dev_data = dev->data;
	I2S_TypeDef *base = (I2S_TypeDef *)dev_cfg->base;
...
static int i2s_rtl8752h_init(const struct device *dev)
{
	const struct i2s_rtl8752h_config *dev_cfg = dev->config;
	struct i2s_rtl8752h_data *dev_data = dev->data;
	I2S_TypeDef *base = (I2S_TypeDef *)dev_cfg->base;
	...
	k_msgq_init(&dev_data->dma_tx.in_queue, (char *)dev_data->tx_in_msgs, sizeof(void *),
		    CONFIG_I2S_RTL8752H_TX_BLOCK_COUNT);
	...
	k_msgq_init(&dev_data->dma_rx.in_queue, (char *)dev_data->rx_in_msgs, sizeof(void *),
		    CONFIG_I2S_RTL8752H_RX_BLOCK_COUNT);
...
static const struct i2s_driver_api i2s_rtl8752h_driver_api = {
	.configure = i2s_rtl8752h_configure,
#if defined(CONFIG_I2S_RTL8752H_RX)
	.read = i2s_rtl8752h_read,
#endif
#if defined(CONFIG_I2S_RTL8752H_TX)
	.write = i2s_rtl8752h_write,
#endif
	.config_get = i2s_rtl8752h_config_get,
	.trigger = i2s_rtl8752h_trigger,
};
...
#define I2S_DMA_CHANNEL_INIT(index, dir)                                                           \
	...
	.dma_cfg =                                                                                 \
		{                                                                                  \
			...
			.dma_callback = i2s_rtl8752h_dma_##dir##_cb,                               \
			.cyclic = true,                                                            \
	},                                                                                         \
	.src_addr_increment =                                                                      \
...
#if defined(CONFIG_I2S_RTL8752H_TX) && defined(CONFIG_I2S_RTL8752H_RX)
#define I2S_DMA_INIT(index) I2S_DMA_CHANNEL(index, tx) I2S_DMA_CHANNEL(index, rx)
#elif defined(CONFIG_I2S_RTL8752H_TX)
#define I2S_DMA_INIT(index) I2S_DMA_CHANNEL(index, tx)
#elif defined(CONFIG_I2S_RTL8752H_RX)
#define I2S_DMA_INIT(index) I2S_DMA_CHANNEL(index, rx)
#endif

#define RTL8752H_I2S_INIT(index)                                                                   \
	static void i2s_rtl8752h_irq_config_func_##index(const struct device *dev);                \
                                                                                                   \
	PINCTRL_DT_INST_DEFINE(index);                                                             \
                                                                                                   \
	static const struct i2s_rtl8752h_config i2s_rtl8752h_cfg_##index = {                       \
		.base = (I2S_TypeDef *)DT_INST_REG_ADDR(index),                                    \
		.clkid = DT_INST_CLOCKS_CELL(index, id),                                           \
		.irq_config_func = i2s_rtl8752h_irq_config_func_##index,                           \
		.pinctrl = PINCTRL_DT_INST_DEV_CONFIG_GET(index),                                  \
	};                                                                                         \
                                                                                                   \
	static struct i2s_rtl8752h_data i2s_rtl8752h_data_##index = {I2S_DMA_INIT(index)};         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(index, &i2s_rtl8752h_init, NULL, &i2s_rtl8752h_data_##index,         \
			      &i2s_rtl8752h_cfg_##index, POST_KERNEL, CONFIG_I2S_INIT_PRIORITY,    \
			      &i2s_rtl8752h_driver_api);                                           \
                                                                                                   \
	static void i2s_rtl8752h_irq_config_func_##index(const struct device *dev)                 \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(index, 0, irq),                                     \
			    DT_INST_IRQ_BY_IDX(index, 0, priority), i2s_rtl8752h_isr,              \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		irq_enable(DT_INST_IRQN(index));                                                   \
	}

DT_INST_FOREACH_STATUS_OKAY(RTL8752H_I2S_INIT)
