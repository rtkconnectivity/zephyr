	...
	if (config->is_enhanced) {
		if (flags & PWM_POLARITY_INVERTED) {
			if (period_cycles == 0U || pulse_cycles == 0U) {
				...
				ENHTIM_SetCCValue((ENHTIM_TypeDef *)timer_base,
		...
		if (flags & PWM_POLARITY_INVERTED) {
			if (period_cycles == 0U || pulse_cycles == 0U) {
				...
				/* duty = 100 */
				TIM_PWMChangeFreqAndDuty((TIM_TypeDef *)timer_base, 0, UINT32_MAX);
			} else {
			...
			if (period_cycles == 0U || pulse_cycles == 0U) {
				...
				TIM_PWMChangeFreqAndDuty((TIM_TypeDef *)timer_base, pulse_cycles,
							 period_cycles - pulse_cycles);
			}
		}
...
static int pwm_bee_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		...
		if (err == -ENOENT) {
			/* sleep status pinctrl is not configured, maintain the level at closest
			 * before sleep status
			 */
...
static const struct pwm_driver_api pwm_bee_driver_api = {
...
#define PWM_BEE_INIT(index)                                                                        \
	...
	static const struct pwm_bee_config pwm_bee_config_##index = {                              \
		...
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(index),                                     \
                                                                                                   ...
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(index, pwm_bee_pm_action);                                        \
	DEVICE_DT_INST_DEFINE(index, &pwm_bee_init, PM_DEVICE_DT_INST_GET(index),                  \
			      &pwm_bee_data_##index, &pwm_bee_config_##index, POST_KERNEL,         \
			      CONFIG_PWM_INIT_PRIORITY, &pwm_bee_driver_api);
