...
static bool bt_hci_bee_recv_cb(T_RTL_BT_HCI_EVT evt, bool status, uint8_t *p_buf, uint32_t len)
{
    int ret = 0;
    ...
    if (ret != 0)
    {
        DBG_DIRECT("[ERR] bt_hci_bee_recv_cb: error, evt %d status %d, type %d, len %d, "
                   "ret %d",
                   evt, status, p_buf[0], len, ret);
        LOG_ERR("bt_hci_bee_recv_cb: error, evt %u status %u, type %u, len %u, ret %d",
                evt, status, p_buf[0], len, ret);
        return false;
    }
...
void bt_hci_bee_handle_rx_data(T_RTL_BT_RX_BUF *p_rx_buf)
{
    const struct device *dev = DEVICE_DT_GET(DT_DRV_INST(0));
    struct bt_hci_bee_data *hci = dev->data;
    ...
    case H4_EVT:
        {
            ...
            discardable = bt_hci_bee_check_hci_event_discardable(&p_rx_buf->p_buf[1]);
...
static void rtl_rx_thread(void *p1, void *p2, void *p3)
{
    ...
    while (1)
    {
        ...
        do
        {
             ...
             * p_rx_buf->p_buf);
             */
            bt_hci_bee_handle_rx_data(p_rx_buf);
...
#define BT_HCI_BEE_DEVICE_INIT(inst)                                                                   \
    static struct bt_hci_bee_data bt_hci_bee_data_##inst = {};                                         \
    DEVICE_DT_INST_DEFINE(inst, NULL, NULL, &bt_hci_bee_data_##inst, NULL, POST_KERNEL,            \
