...
static bool spi_rtl87x2g_tx_fifo_not_full(const struct spi_rtl87x2g_config *cfg)
{
#if defined(CONFIG_SOC_SERIES_RTL87X2G)
...
static bool spi_rtl87x2g_rx_fifo_not_full(const struct spi_rtl87x2g_config *cfg)
{
#if defined(CONFIG_SOC_SERIES_RTL87X2G)
...
static bool spi_rtl87x2g_rx_fifo_empty(const struct spi_rtl87x2g_config *cfg)
{
	return SPI_GetRxFIFOLen((SPI_TypeDef *)cfg->reg) == 0;
}

static bool spi_rtl87x2g_context_ongoing(const struct device *dev)
{
...
static bool spi_rtl87x2g_hw_ongoing(const struct device *dev)
{
...
static bool spi_rtl87x2g_transfer_ongoing(const struct device *dev)
{
	return spi_rtl87x2g_context_ongoing(dev) || spi_rtl87x2g_hw_ongoing(dev);
}
...
static int spi_rtl87x2g_frame_exchange(const struct device *dev)
{
	...
	while (spi_rtl87x2g_tx_fifo_not_full(dev_config) &&
	       spi_rtl87x2g_rx_fifo_not_full(dev_config)) {
		if (ctx->tx_len) {
			if (datalen <= 8) {
	...
	while (!spi_rtl87x2g_rx_fifo_empty(dev_config)) {
...
static void spi_rtl87x2g_isr(struct device *dev)
{
	...
	if (spi_rtl87x2g_transfer_ongoing(dev)) {
		err = spi_rtl87x2g_frame_exchange(dev);
	}

	if (err || !spi_rtl87x2g_transfer_ongoing(dev)) {
		spi_rtl87x2g_complete(dev, err);
	}
}
...
void spi_rtl87x2g_dma_rx_cb(const struct device *dev, void *user_data, uint32_t channel, int status)
{
	...
	if (spi_rtl87x2g_chunk_transfer_finished(spi_dev)) {
		...
		if (spi_rtl87x2g_context_ongoing(spi_dev)) {
			...
			spi_rtl87x2g_complete(spi_dev, 0);
			return;
		}
	}
...
void spi_rtl87x2g_dma_tx_cb(const struct device *dev, void *user_data, uint32_t channel, int status)
{
}

#endif /* CONFIG_SPI_RTL87X2G_DMA */

static int spi_rtl87x2g_configure(const struct device *dev, const struct spi_config *spi_cfg)
{
	struct spi_rtl87x2g_data *data = dev->data;
	const struct spi_rtl87x2g_config *config = dev->config;
...
static int spi_rtl87x2g_transceive_impl(const struct device *dev, const struct spi_config *spi_cfg,
					...
					spi_callback_t cb, void *userdata)
{
	...
	if (!asynchronous) {
		...
		} while (spi_rtl87x2g_transfer_ongoing(dev));

#else /* defined(CONFIG_SPI_ASYNC) && defined(CONFIG_SPI_RTL87X2G_INTERRUPT) */
...
static int spi_rtl87x2g_transceive(const struct device *dev, const struct spi_config *spi_cfg,
				   const struct spi_buf_set *tx_bufs,
...
static int spi_rtl87x2g_transceive_async(const struct device *dev, const struct spi_config *spi_cfg,
					 const struct spi_buf_set *tx_bufs,
...
static int spi_rtl87x2g_release(const struct device *dev, const struct spi_config *config)
{
	struct spi_rtl87x2g_data *data = dev->data;
...
#ifdef CONFIG_PM_DEVICE
static int spi_rtl87x2g_pm_action(const struct device *dev, enum pm_device_action action)
{
	struct spi_rtl87x2g_data *data = dev->data;
	const struct spi_rtl87x2g_config *config = dev->config;
	int err;
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		if (data->initialized) {
			data->initialized = false;
			spi_rtl87x2g_configure(dev, data->ctx.config);
		}
...
#define SPI_RTL87X2G_IRQ_CONFIGURE(index)                                                          \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), spi_rtl87x2g_isr,   \
...
#define RTL87X2G_SPI_INIT(index)                                                                   \
	...
	static const struct spi_rtl87x2g_config spi_rtl87x2g_config_##index = {                    \
	...
	DEVICE_DT_INST_DEFINE(index, &spi_rtl87x2g_init, PM_DEVICE_DT_INST_GET(index),             \
			      ...
			      POST_KERNEL, CONFIG_SPI_INIT_PRIORITY, &spi_rtl87x2g_driver_api);
