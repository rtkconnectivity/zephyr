...
#if DT_HAS_COMPAT_STATUS_OKAY(realtek_bee_udc)
#include "trace.h"
#define DT_DRV_COMPAT snps_dwc2

#define UDC_DT_WRAPPER_REG_ADDR(n) UINT_TO_POINTER(DT_INST_REG_ADDR_BY_NAME(n, wrapper))

static int bee_udc_init(const struct device *dev)
{
...
static int bee_udc_pre_enable(const struct device *dev)
{

	return 0;
}

static int bee_udc_post_enable(const struct device *dev)
{

	return 0;
}

static int bee_udc_disable(const struct device *dev)
{
...
static int bee_udc_shutdown(const struct device *dev)
{

	return 0;
}

static int bee_udc_irq_clear(const struct device *dev)
{

	return 0;
}

static int bee_udc_caps(const struct device *dev)
{
	struct udc_data *data = dev->data;
...
static int bee_udc_is_phy_clk_off(const struct device *dev)
{

	return 0;
}

#define QUIRK_BEE_UDC_DEFINE(n)                                                                    \
	struct dwc2_vendor_quirks dwc2_vendor_quirks_##n = {                                       \
		.init = bee_udc_init,                                                              \
		.pre_enable = bee_udc_pre_enable,                                                  \
		.post_enable = bee_udc_post_enable,                                                \
		.disable = bee_udc_disable,                                                        \
		.shutdown = bee_udc_shutdown,                                                      \
		.irq_clear = bee_udc_irq_clear,                                                    \
		.caps = bee_udc_caps,                                                              \
		.is_phy_clk_off = bee_udc_is_phy_clk_off,                                          \
	};

DT_INST_FOREACH_STATUS_OKAY(QUIRK_BEE_UDC_DEFINE)

#undef DT_DRV_COMPAT

#endif /*DT_HAS_COMPAT_STATUS_OKAY(realtek_bee_udc) */
