...
LOG_MODULE_REGISTER(rtc_rtl87x2g, CONFIG_RTC_LOG_LEVEL);

#ifdef CONFIG_RTC_ALARM
struct rtc_rtl87x2g_ch_data {
...
struct rtc_rtl87x2g_updata_data {
...
struct rtc_rtl87x2g_data {
	...
	struct k_mutex lock;
#ifdef CONFIG_RTC_UPDATE
	struct rtc_rtl87x2g_updata_data update;
#endif
#ifdef CONFIG_RTC_ALARM
	struct rtc_rtl87x2g_ch_data alarm[];
#endif
};

struct rtc_rtl87x2g_config {
	uint32_t reg;
	uint32_t src_freq;
	uint32_t prescaler;
	uint8_t channels;
...
{
	struct rtc_rtl87x2g_data *data = dev->data;

	return cnt / data->freq;
}

#ifdef CONFIG_RTC_ALARM
static int rtc_rtl87x2g_alarm_set_time(const struct device *dev, uint16_t id, uint16_t mask,
...
static int rtc_rtl87x2g_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	const struct rtc_rtl87x2g_config *cfg = dev->config;
	struct rtc_rtl87x2g_data *data = dev->data;
	...
	for (uint8_t i = 0; i < cfg->channels; i++) {
		if (data->alarm[i].enabled) {
			rtc_rtl87x2g_alarm_set_time(dev, i, data->alarm[i].mask,
...
static int rtc_rtl87x2g_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	struct rtc_rtl87x2g_data *data = dev->data;
...
#ifdef CONFIG_RTC_ALARM
static int rtc_rtl87x2g_alarm_get_supported_fields(const struct device *dev, uint16_t id,
						   uint16_t *mask)
{
	const struct rtc_rtl87x2g_config *cfg = dev->config;
				       ...
				       const struct rtc_time *timeptr)
{
	...
	rtc_rtl87x2g_get_time(dev, &cur_time);
...
static int rtc_rtl87x2g_alarm_get_time(const struct device *dev, uint16_t id, uint16_t *mask,
...
static int rtc_rtl87x2g_alarm_is_pending(const struct device *dev, uint16_t id)
{
	struct rtc_rtl87x2g_data *data = dev->data;
...
static int rtc_rtl87x2g_alarm_set_callback(const struct device *dev, uint16_t id,
...
#ifdef CONFIG_RTC_UPDATE
static int rtc_rtl87x2g_update_set_callback(const struct device *dev, rtc_update_callback callback,
					    void *user_data)
{
	struct rtc_rtl87x2g_data *data = dev->data;
...
struct rtc_driver_api rtc_rtl87x2g_driver_api = {
	.set_time = rtc_rtl87x2g_set_time,
	.get_time = rtc_rtl87x2g_get_time,
#ifdef CONFIG_RTC_ALARM
	.alarm_get_supported_fields = rtc_rtl87x2g_alarm_get_supported_fields,
	.alarm_set_time = rtc_rtl87x2g_alarm_set_time,
	.alarm_get_time = rtc_rtl87x2g_alarm_get_time,
	.alarm_is_pending = rtc_rtl87x2g_alarm_is_pending,
	.alarm_set_callback = rtc_rtl87x2g_alarm_set_callback,
#endif /* CONFIG_RTC_ALARM */
#ifdef CONFIG_RTC_UPDATE
	.update_set_callback = rtc_rtl87x2g_update_set_callback,
...
{
	struct rtc_rtl87x2g_data *data = dev->data;
	struct rtc_rtl87x2g_ch_data *alarm = &data->alarm[chan];
...
static int rtc_rtl87x2g_init(const struct device *dev)
{
	const struct rtc_rtl87x2g_config *cfg = dev->config;
	struct rtc_rtl87x2g_data *data = dev->data;
...
#ifdef CONFIG_RTC_ALARM
#define RTC_RTL87X2G_ALARM_INIT(index) .alarm[DT_INST_PROP(index, channels)] = {0},
#else
#define RTC_RTL87X2G_ALARM_INIT(index)
#endif

#ifdef CONFIG_RTC_UPDATE
#define RTC_RTL87X2G_UPDATE_INIT                                                                   \
	.update = {                                                                                \
		.callback = NULL,                                                                  \
		.user_data = NULL,                                                                 \
		.enabled = false,                                                                  \
...
#define RTC_RTL87X2G_INIT(index)                                                                   \
	RTC_IRQ_CONFIG(index);                                                                     \
	static struct rtc_rtl87x2g_data rtc_rtl87x2g_data_##index = {                              \
		RTC_RTL87X2G_UPDATE_INIT RTC_RTL87X2G_ALARM_INIT(index)};                          \
	static const struct rtc_rtl87x2g_config rtc_rtl87x2g_config_##index = {                    \
		...
		.get_irq_pending = get_irq_pending_##index,                                        \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(index, rtc_rtl87x2g_init, NULL, &rtc_rtl87x2g_data_##index,          \
			      &rtc_rtl87x2g_config_##index, PRE_KERNEL_1,                          \
			      CONFIG_RTC_INIT_PRIORITY, &rtc_rtl87x2g_driver_api);

DT_INST_FOREACH_STATUS_OKAY(RTC_RTL87X2G_INIT);
