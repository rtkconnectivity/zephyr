...
static uint32_t     sTransmitRetry = 0;

// rx ack frame
static uint8_t sAckWaitingSeq = 0;
static rx_item_t sAckItem;

// tx ack frame
static volatile bool sAckedWithFramePending = false;

// tx enhack frame
static uint8_t sEnhAckPsdu[MAC_FRAME_TX_HDR_LEN + IEEE802154_MAX_LENGTH];
static volatile uint8_t enhack_frm_len = 0;
static volatile bool    enhack_frm_sec_en = false;
...
static uint32_t      sCslPeriod = 0;
static uint32_t      sCslSampleTime = 0;
static uint8_t       sCslIeIndex = 0;

static uint8_t       enhAckProbingDataLen = 0;
static bool          enhAckProbingWithLqi = false;
static bool          enhAckProbingWithMargin = false;
static bool          enhAckProbingWithRssi = false;
static uint8_t       sVendorIeIndex = 0;
...
static uint16_t getCslPhase()
{
    ...
    uint32_t diff = (cslPeriodInUs - (curTime % cslPeriodInUs) + (sCslSampleTime % cslPeriodInUs)) %
                    cslPeriodInUs;
...
bool readFrame(struct rtl87x2g_802154_rx_frame *receivedFrame)
{
    ...
    if (enhack_frm_len > 0 && mac_GetTxEnhAckPending())
    {
        ...
        if (MAC_STS_SUCCESS == mac_TrigTxEnhAck_patch(false, enhack_frm_sec_en))
        {
            enhack_frm_len = 0;
            if (enhack_frm_sec_en)
            {
    ...
    receivedFrame->time = k_ticks_to_us_floor64(k_uptime_ticks()) -
                          (p_data[0] * 32) -
...
void readAck(struct rtl87x2g_802154_rx_frame *receivedFrame)
{
    ...
    receivedFrame->rssi = mac_GetRSSIFromRaw(prx_fifo_tail->rssi,
                                             mac_GetChannel());
...
static uint8_t generate_ieee_enhack_frame(uint8_t *txbuf, uint8_t buf_len, fc_t *fc, uint8_t seq,
                                          uint16_t dpid, uint8_t *dadr, uint8_t aux_len, uint8_t *aux)
{
    uint8_t len = 0;
    csl_ie_t *p_csl_ie;
    vendor_ie_t *p_vendor_ie;
...
#define RTL87X2G_802154_DATA(dev) \
	((struct rtl87x2g_802154_data * const)(dev)->data)

#define RTL87X2G_802154_CFG(dev) \
	((const struct rtl87x2g_802154_config * const)(dev)->config)
...
static void rtl87x2g_rx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		...
		//DBG_DIRECT("Frame received %d %d", pkt_len, pkt_seq);
		 ...
		 */
		pkt = net_pkt_rx_alloc_with_buffer(rtl87x2g_radio->iface, pkt_len,
		...
		if (net_recv_data(rtl87x2g_radio->iface, pkt) < 0) {
			DBG_DIRECT("Packet dropped by NET stack");
			goto drop;
		}
...
static int rtl87x2g_energy_scan_start(const struct device *dev,
				  uint16_t duration,
...
static int rtl87x2g_set_ieee_addr(const struct device *dev,
...
static int rtl87x2g_filter(const struct device *dev, bool set,
...
static int handle_ack(struct rtl87x2g_802154_data *rtl87x2g_radio)
{
	...
	ack_pkt = net_pkt_rx_alloc_with_buffer(rtl87x2g_radio->iface, ack_len,
					       AF_UNSPEC, 0, K_NO_WAIT);
	if (!ack_pkt) {
		DBG_DIRECT("No free packet available.");
		err = -ENOMEM;
		goto free_drv_ack;
	}
	 ...
	 * PHY header (1 byte).
	 */
	if (net_pkt_write(ack_pkt, rtl87x2g_radio->ack_frame.psdu + 1,
			  ack_len) < 0) {
		DBG_DIRECT("Failed to write to ack_pkt.");
		err = -ENOMEM;
		goto free_net_ack;
	}
	...
	if (ieee802154_handle_ack(rtl87x2g_radio->iface, ack_pkt) != NET_OK) {
		DBG_DIRECT("ACK packet not handled - releasing.");
	}
...
static void rtl87x2g_tx_started(const struct device *dev,
			    struct net_pkt *pkt,
			    struct net_buf *frag)
{
	ARG_UNUSED(pkt);
	...
	if (rtl87x2g_data.event_handler) {
		rtl87x2g_data.event_handler(dev, IEEE802154_EVENT_TX_STARTED,
					(void *)frag);
	}
}

static int rtl87x2g_tx(const struct device *dev,
...
{
	ARG_UNUSED(dev);
    //DBG_DIRECT("%s: enter receive", __func__);
	return 0;
}
...
{
	ARG_UNUSED(dev);
    //DBG_DIRECT("%s: enter sleep", __func__);
	return 0;
}
...
static int rtl87x2g_init(const struct device *dev)
{
	...
	k_thread_create(&rtl87x2g_radio->rx_thread, rtl87x2g_radio->rx_stack,
			RTL87X2G_RX_STACK_SIZE,
			rtl87x2g_rx_thread, rtl87x2g_radio, NULL, NULL,
...
static int rtl87x2g_configure(const struct device *dev,
...
static int rtl87x2g_attr_get(const struct device *dev,
...
void Process_RadioInt(uint32_t int_status)
{
    ...
    if (status.b.rxelyif)
    {
        ...
        do
        {
            if (FRAME_VER_2015 == mac_GetRxFrmVersion())
            {
                ...
                fc_ack.ie_present = 0;
                if (sCslPeriod > 0) { fc_ack.ie_present = 1; }
                if (enhAckProbingDataLen > 0) { fc_ack.ie_present = 1; }
                ...
                if (fc_ack.dst_addr_mode == ADDR_MODE_SHORT)
                {
                    ...
                    enhack_frm_len = generate_ieee_enhack_frame(&sEnhAckPsdu[MAC_FRAME_TX_HDR_LEN],
    ...
    if (status.b.rxif)
    {
        if (FRAME_TYPE_ACK == mac_GetRxFrmType())
        {
...
static struct ieee802154_radio_api rtl87x2g_radio_api = {
	.iface_api.init = rtl87x2g_iface_init,

	.get_capabilities = rtl87x2g_get_capabilities,
	.cca = rtl87x2g_cca,
...
NET_DEVICE_DT_INST_DEFINE(0, rtl87x2g_init, NULL, &rtl87x2g_data, &rtl87x2g_radio_cfg,
			  RTL87X2G_INIT_PRIO, &rtl87x2g_radio_api, L2,
			  L2_CTX_TYPE, MTU);
#else
DEVICE_DT_INST_DEFINE(0, rtl87x2g_init, NULL, &rtl87x2g_data, &rtl87x2g_radio_cfg,
		      POST_KERNEL, RTL87X2G_INIT_PRIO,
		      &rtl87x2g_radio_api);
#endif
