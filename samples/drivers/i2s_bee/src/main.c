...
const struct device *dev_uart = DEVICE_DT_GET_OR_NULL(DT_ALIAS(dump_mic));
const struct device *dev_i2s = DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_i2s));
const struct device *dev_codec = DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_codec));
...
#define TIMEOUT_SECONDS K_SECONDS(5)

static void i2s_timer_timeout(struct k_timer *timer);
K_TIMER_DEFINE(i2s_timer, i2s_timer_timeout, NULL);

uint8_t rx_buf[BLOCK_SIZE] __aligned(2);
uint8_t tx_buf[BLOCK_SIZE] __aligned(2);
...
static void i2s_codec_rx_sample(void)
{
	...
	static audio_property_value_t val;

	if (!dev_codec) {
		printf("CODEC device not available on this board\n");
		return;
	}
	...
	val.vol = 0x2f;
	ret = audio_codec_set_property(dev_codec, AUDIO_PROPERTY_OUTPUT_VOLUME, channel, val);
	if (ret) {
		printf("[%s] ret%d line%d\n", __func__, ret, __LINE__);
	}
...
static void i2s_codec_tx_sample(void)
{
	int ret;
	...
	if (!dev_codec) {
		printf("CODEC not available on this board, skip %s\n", __func__);
		return;
	}
	...
	codec_cfg.dai_cfg.i2s.frame_clk_freq = 16000;
	codec_cfg.dai_cfg.i2s.word_size = 16;
	/* channels == 0 for i2s codec tx */
	codec_cfg.dai_cfg.i2s.channels = 0;
	codec_cfg.dai_cfg.i2s.format = I2S_FMT_DATA_FORMAT_I2S | I2S_FMT_DATA_ORDER_MSB;
	codec_cfg.dai_cfg.i2s.options =
		I2S_OPT_BIT_CLK_CONT | I2S_OPT_BIT_CLK_CONTROLLER |
		I2S_OPT_FRAME_CLK_CONTROLLER | I2S_OPT_PINGPONG;
	...
	codec_cfg.dai_cfg.i2s.block_size = BLOCK_SIZE;
	codec_cfg.dai_cfg.i2s.mem_slab = &tx_mem_slab;
	codec_cfg.dai_cfg.i2s.timeout = SYS_FOREVER_MS;
	...
	ret = i2s_configure(dev_i2s, I2S_DIR_TX, &codec_cfg.dai_cfg.i2s);
	if (ret) {
		printf("[%s] ret%d line%d\n", __func__, ret, __LINE__);
	}
...
int main(void)
{
	printf("I2S Bee sample started\n");
	i2s_codec_rx_sample();

#if defined(CONFIG_SOC_SERIES_RTL87X2G)
	/* I2S + Internal CODEC TX tests for RTL87x2G.
	 * Only RTL87x2G Internal CODEC supports TX.
	 * RTL8752H/RTL87X2J Internal CODEC un-supports TX.
	 */
	i2s_codec_tx_sample();
#endif

#if defined(CONFIG_SOC_SERIES_RTL8752H) || defined(CONFIG_SOC_SERIES_RTL87X2J)
	/* I2S TX tests for RTL8752H/RTL87X2J. */
	i2s_tx_sample();
#endif
