...
static void thread1(void *arg)
{
	...
	zassert_equal(status, true,
		      "notify wait failed unexpectedly");
...
ZTEST(osif_task_signal, test_task_notified)
{
	...
	status = os_task_create(&id2, "task_ntf",  thread1,
...
void test_signal_from_isr(void *param)
{
	...
	zassert_equal(status, true,
		      "notify wait from ISR failed unexpectedly");
	zassert_equal((p_notify & EXPECTED_NOTIFY_VAL),
		       EXPECTED_NOTIFY_VAL, "unexpected notify value from ISR");
}
...
ZTEST(osif_task_signal, test_notify_from_isr)
{
	...
	bool status;
	status = os_task_create(&id3, "test_isr", test_signal_from_isr,
                        &task_param, STACKSZ, 6);
