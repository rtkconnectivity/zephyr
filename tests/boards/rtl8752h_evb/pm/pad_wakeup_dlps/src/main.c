...
ZTEST(pad_wakeup_dlps, test_gpio_wakeup_dlps)
{
	...
	/* GPIO_INT_EDGE_TO_ACTIVE:When dlps is turned on, multiple groups will be triggered
	 * repeatedly, and when dlps
	 ...
	 */
	err = gpio_pin_interrupt_configure(button_dev, BUTTON_PIN, GPIO_INT_LEVEL_ACTIVE);
