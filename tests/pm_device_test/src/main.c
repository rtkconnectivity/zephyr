...
typedef bool (*POWERCheckFunc)(void);
...
#ifdef CONFIG_COUNTER
	const struct device *counter_dev = DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_counter_timer));
...
static int shell_pm_test_gpio(const struct shell *sh, size_t argc, char **argv)
{
	...
	pm_gpio_do_one_round("connect input pin to output pin to wakeup");
	pm_gpio_do_one_round("disconnect input pin to output pin to wakeup");
#endif /* CONFIG_BEE_GPIO_SUPPORT_BOTH_EDGE */
...
#ifdef CONFIG_SPI
	static struct spi_dt_spec spi_spec = SPI_DT_SPEC_GET(SPI_DEV, SPI_OP_CFG(FRAME_SIZE), 0);
...
static int shell_pm_test_rtc(const struct shell *sh, size_t argc, char **argv)
{
	...
	rtc_alarm_set_time(rtc_dev, 0, 0x1ff, &test_alarm_time_set);
...
#if defined(CONFIG_SENSOR) && (defined(CONFIG_AON_QDEC_BEE) || defined(CONFIG_LPQDEC_BEE) ||       \
			       defined(CONFIG_AON_QDEC_RTL87X3G))
...
static int shell_pm_test_sdhc(const struct shell *sh, size_t argc, char **argv)
{
	...
	static const struct device *const sdhc_dev_sdmmc =
		DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_sdmmc));
	static const struct device *const sdhc_dev_sdio =
		DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_sdio));
...
#ifdef CONFIG_CAN
	const struct device *const can_dev = DEVICE_DT_GET_OR_NULL(DT_ALIAS(test_can));
...
#define SHELL_CMD_ARG_CREATE                                                                       \
	SHELL_CMD_ARG(uart, NULL, "uart pm test", shell_pm_test_uart, 0, 0),                       \
		...
		SHELL_CMD_ARG(aon_qdec, NULL, "aon qdec pm test", shell_pm_test_aon_qdec, 0, 0),   \
