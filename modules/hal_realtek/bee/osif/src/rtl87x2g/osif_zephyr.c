...
void os_mem_aligned_free_zephyr(void *block)
{
	...
	memcpy(&ptr, (uint8_t *)block - sizeof(void *), sizeof(void *));
	os_mem_free_zephyr(ptr);
}
...
{
	return k_is_pre_kernel() ? true : false;
}

/************************************************************
 * TASK / THREAD MANAGEMENT
 ************************************************************/
K_MEM_SLAB_DEFINE(osif_task_slab, sizeof(struct osif_task), CONFIG_OSIF_TASK_MAX_COUNT, 4);
			   ...
			   void *param, uint16_t stack_size, uint16_t priority)
{
	...
	if (priority > CONFIG_OSIF_TASK_MAX_PRIORITY) {
		LOG_ERR("Invalid priority. OSIF Task Priority is expected to 0 ~ 6.");
		return false;
	}

	if (k_mem_slab_alloc(&osif_task_slab, (void **)&task, K_NO_WAIT) != 0) {
		LOG_ERR("Exceed Max number of tasks: %d!", CONFIG_OSIF_TASK_MAX_COUNT);
		return false;
	}
	...
	if (name && strcmp(name, "low_stack_task") == 0) {
		...
		zephyr_prio = CONFIG_OSIF_TASK_MAX_PRIORITY - priority;
	}
...
bool os_task_delete_zephyr(void *handle)
{
	...
	/*
	 * Note: Memory cleanup (slab free & stack free) is tricky here because
	 * the thread might be aborting itself. In a robust system, you might need
	 * a cleanup workqueue or similar mechanism.
	 */
	return true;
}
...
bool os_task_priority_get_zephyr(void *handle, uint16_t *priority)
{
	...
	*priority = (uint16_t)(CONFIG_OSIF_TASK_MAX_PRIORITY - zephyr_prio);
...
bool os_task_priority_set_zephyr(void *handle, uint16_t priority)
{
	...
	thread = task ? &task->zthread : k_current_get();
	switch_priority = CONFIG_OSIF_TASK_MAX_PRIORITY - priority;
...
bool os_task_notify_take_zephyr(long clear_count_on_exit, uint32_t wait_ticks, uint32_t *notify)
{
	...
	__ASSERT(task->poll_event.state == K_POLL_STATE_SIGNALED, "event state not signalled!");
...
bool os_task_signal_recv_zephyr(uint32_t *signal, uint32_t wait_ms)
{
	ARG_UNUSED(signal);
	ARG_UNUSED(wait_ms);
 ...
 * SEMAPHORE & MUTEX
 ************************************************************/
K_MEM_SLAB_DEFINE(osif_sem_slab, sizeof(struct k_sem), CONFIG_OSIF_SEM_MAX_COUNT, 4);
			  ...
			  uint32_t max_count)
{
	...
	if (k_mem_slab_alloc(&osif_sem_slab, (void **)&sem, K_NO_WAIT) != 0) {
		LOG_ERR("Exceed Max number of semaphores: %d!", CONFIG_OSIF_SEM_MAX_COUNT);
		return false;
	}
...
K_MEM_SLAB_DEFINE(osif_mutex_slab, sizeof(struct k_mutex), CONFIG_OSIF_MUTEX_MAX_COUNT, 4);
...
bool os_mutex_create_zephyr(void **handle_ptr)
{
	...
	if (k_mem_slab_alloc(&osif_mutex_slab, (void **)&mutex, K_NO_WAIT) != 0) {
		LOG_ERR("Exceed Max number of mutexs: %d!", CONFIG_OSIF_MUTEX_MAX_COUNT);
		return false;
	}
 ...
 * MESSAGE QUEUE
 ************************************************************/
K_MEM_SLAB_DEFINE(osif_msgq_slab, sizeof(struct k_msgq), CONFIG_OSIF_MSGQ_MAX_COUNT, 4);
				       ...
				       uint32_t msg_size, const char *func, uint32_t line)
{
	...
	if (k_mem_slab_alloc(&osif_msgq_slab, (void **)&msgq, K_NO_WAIT) != 0) {
		LOG_ERR("Exceed Max number of msgqs: %d!", CONFIG_OSIF_MSGQ_MAX_COUNT);
		return false;
	}
 ...
 * SOFTWARE TIMER
 ************************************************************/
K_MEM_SLAB_DEFINE(osif_timer_slab, sizeof(struct osif_timer), CONFIG_OSIF_TIMER_MAX_COUNT,
		  __alignof__(struct osif_timer));
			    ...
			    uint32_t interval_ms, bool reload, void (*timer_callback)())
{
	...
	if (k_mem_slab_alloc(&osif_timer_slab, (void **)&timer, K_NO_WAIT) != 0) {
		LOG_ERR("Exceed Max number of timers: %d!", CONFIG_OSIF_TIMER_MAX_COUNT);
		return false;
	}
	memset(timer, 0, sizeof(struct osif_timer));
	...
	*handle_ptr = timer;
	...
	for (i = 0; i < CONFIG_OSIF_TIMER_MAX_COUNT; i++) {
	...
	if (!stored) {
		...
		*handle_ptr = NULL;
		LOG_ERR("Failed to store timer in pool because it is full.");
	}
...
bool os_timer_get_timer_number_zephyr(void **handle_ptr, uint8_t *timer_num)
{
	...
	LOG_ERR("Timer handle %p not found in pool", timer);
	return false;
}
